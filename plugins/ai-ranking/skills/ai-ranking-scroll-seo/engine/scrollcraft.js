/* ============================================================================
   ...
   ---------------------------------------------------------------------------
     ...
     data-sc-count="0 4200"   number bloom across the cue window. Add
                              data-sc-count-locale="auto" to format with the
                              page language (15.000 es / 15,000 en), and
                              data-sc-count-step="500" to climb in real steps.
                              Outside any
...
(function (global) {
  ...
  // ---- number formatting --------------------------------------------------
  // data-sc-count-locale="auto" (nearest [lang]) or a BCP 47 tag ("es-AR")
  // formats with Intl, so "15.000" in Spanish and "15,000" in English both come
  // out right. Write the attribute's numbers as plain digits in that case.
  function countLocale(el) {
    var l = el && el.getAttribute && el.getAttribute('data-sc-count-locale');
    if (!l) return null;
    if (l === 'auto') { var h = el.closest('[lang]'); l = h ? h.getAttribute('lang') : 'en'; }
    return l;
  }
  function formatNum(v, template, el) {
    // data-sc-count-step="500" snaps the running value, so a price climbs in
    // real instalment increments instead of showing 5,545.
    var step = el && el.getAttribute && parseFloat(el.getAttribute('data-sc-count-step'));
    if (step > 0) v = Math.round(v / step) * step;
    var loc = countLocale(el);
    // useGrouping 'always': es/pt locales skip the separator on 4-digit numbers
    // ("3000" beside "15.000"), which reads as two different formats.
    if (loc) return new Intl.NumberFormat(loc, { maximumFractionDigits: 0, useGrouping: 'always' }).format(Math.round(v));
    var decimals = 0;
  ...
  function mount(root, opts) {
    ...
    (function () {
      ...
      function run(c) {
        ...
        if (reduce || ms <= 0) { c.textContent = formatNum(b, tpl, c); return; }
        var t0 = null, last = null;
        function frame(now) {
          if (t0 === null) t0 = now;
          var t = Math.min((now - t0) / ms, 1);
          var out = formatNum(a + (b - a) * ease(t), tpl, c);
          if (out !== last) { c.textContent = out; last = out; }
          if (t < 1) requestAnimationFrame(frame);
        }
        requestAnimationFrame(frame);
      }
      els.forEach(function (c) { var n = spec(c); c.textContent = formatNum(num(n[0]), n[1] || '0', c); });
      if ('IntersectionObserver' in window) {
    ...
    function read() {
      ...
      for (var i = 0; i < acts.length; i++) {
        ...
        for (var ct = 0; ct < a.counts.length; ct++) {
          ...
          var out = formatNum(val, K.tpl, K.el);
