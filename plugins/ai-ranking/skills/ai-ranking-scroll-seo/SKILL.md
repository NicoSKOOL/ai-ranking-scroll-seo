---
name: ai-ranking-scroll-seo
description: >
  Build premium scroll-driven websites that also rank: dimensional layered
  heroes, a bespoke signature move, restrained scroll motion, AND hard SEO gates
  (WebP-only images inside byte budgets, JSON-LD schema, the researched keyword
  in the title and H1, Lighthouse 95+ on performance, accessibility, best
  practices and SEO on mobile and desktop, and agentic browsing readiness with
  llms.txt and WebMCP). Multi-language via Astro. Generates photoreal assets via
  fal.ai (GPT Image 2.5) or kie.ai, captioned as illustrative in trust mode.
  Use for "scroll site", "scroll animation website", "premium hero", "layered
  hero", "cinematic landing page", "scrollytelling", "Apple-style landing page",
  "a site that feels like an experience", or any request for a wow website that
  must also load fast and rank.
...
## The five gates (AI Ranking additions)

A page is not done when it looks good. It is done when these five pass, on every
language route, mobile and desktop. Read [references/seo-performance.md](references/seo-performance.md)
before Step 1 and plan for them from the start; retrofitting speed onto a
finished scroll page is the expensive way.

1. **Speed and images:** WebP only, responsive `srcset`, width/height on every
   image, phone-size files inside budgets (`optimize-images.mjs`).
2. **Schema:** JSON-LD graph for the business type, nothing claimed that is not
   visible ([references/schema.md](references/schema.md)).
3. **Keywords:** one researched primary keyword per language in the `<title>`
   and the single `<h1>` (`verify-seo.mjs`).
4. **Lighthouse ≥ 95** on all four categories (`lighthouse-gate.mjs`).
5. **Agentic browsing:** content without JS, real controls, llms.txt, WebMCP on
   the main action ([references/agentic.md](references/agentic.md)).
...
before planning a premium marketing site. It distills the ten-site reference rebuild
...
**fal.ai route (default for this skill):** `FAL_KEY` in the env or a `.env`.
GPT Image 2.5 Flare produces genuine alpha with `--transparent`, which is what
layered hero planes need:

```bash
node <skill>/scripts/fal.mjs still "$(cat assets/STYLE.txt)\n\nBACK PLATE ONLY: ..." assets/masters/hero-back.png --size 2048x1152 --manifest /tmp/m/back.json
...
writes to one manifest lose entries. Composite the planes over magenta to
inspect the alpha before building. Crop portrait `-m` masters for the phone hero
(see seo-performance.md §2).
...
Then run the gates (seo-performance.md): `verify-seo.mjs` on the build,
deploy to staging, `lighthouse-gate.mjs --staging --runs 3` on every language
route. A gate failure is fixed in the page AND, if it was a gap in this skill,
in the skill (reference text, script or engine) with a CHANGELOG entry.

`shoot.mjs` waits for the engine, so it cannot shoot the static mode. Check
reduced motion and no-JS with a plain Playwright full-page screenshot
(`reducedMotion: 'reduce'`, `javaScriptEnabled: false`).
...
| Content hidden until JS runs, for everyone | Gate hiding on `html.sc-js`; the default CSS is a complete static document |
| A section class that sets `position` on a `[data-sc-stage]` | Leave stage positioning to the engine; the page CSS forces `position: sticky` under `.sc-js` |
| A keyword chosen by feel | Search volumes per language first, then the H1 |
| A 16:9 hero plane served to a portrait phone | `<picture>` with a portrait `-m` crop at 640w |
| Any JPG/PNG in the build | WebP via `optimize-images.mjs` |
| A generated place/product shown without "illustrative" | Trust-mode caption |
| Lighthouse measured once on a cold cache | `--runs 3` median after one warming request |
...
each existing row, the five gate results (Lighthouse table per route and form
factor, verify-seo summary), the journey, the feeling curve and the peak, the feel-check
